...
def comment():
    ...
    if request.method == 'POST' and comment_form.validate():
        ...
        db.session.commit()
        success_message = 'Nuevo comentario añadido!'
...
def cookie():
    ...
    response.set_cookie('custome_cookie', 'username')
    return response
...
def reviews(page = 1):
    per_page = 3
    comment_list = Comment.query.join(User).add_columns(User.username, Comment.text, Comment.created_date).paginate(page,per_page,True)
    return render_template('reviews.html', comments = comment_list, current_page = page, date_format = date_format )
